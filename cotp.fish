...
complete -F cotp -n "not __fish_seen_subcommand_from $commands" -s d -l database-path  -r -d "Set the database path"
...
#add
complete -x cotp -n "__fish_seen_subcommand_from add" -s a -l algorithm -a "sha1 sha256 sha512 md5"      -d "OTP Algorithm"
complete -x cotp -n "__fish_seen_subcommand_from add" -s t -l type      -a "totp hotp steam yandex motp" -d "Specify the OTP code type"
...
#edit
complete -x cotp -n "__fish_seen_subcommand_from edit" -s a -l algorithm -a "sha1 sha256 sha512 md5" -d "OTP algorithm"
